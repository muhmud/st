...
static double defaultfontsize = 19;
...
void xdrawcursor(int cx, int cy, Glyph g, int ox, int oy, Glyph og, Line line, int len) {
  ...
  if (IS_SET(MODE_FOCUSED)) {
    ...
    case 5: /* blinking bar */
      ...
      /* FALLTHROUGH */
    case 6: /* steady bar */
      XftDrawRect(xw.draw, &drawcol, borderpx + cx * win.cw, borderpx + cy * win.ch,
    ...
    XftDrawRect(xw.draw, &drawcol, borderpx + (cx + 1) * win.cw - 1, borderpx + cy * win.ch, 1,
                win.ch - 1);
    XftDrawRect(xw.draw, &drawcol, borderpx + cx * win.cw, borderpx + (cy + 1) * win.ch - 1, win.cw,
                1);
  }
}
